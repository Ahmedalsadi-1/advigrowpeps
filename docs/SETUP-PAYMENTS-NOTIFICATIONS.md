...
Site: advigrow.online. Contact: orders@advigrow.online, coa@advigrow.online.

## 1. Stripe (card payments)
1. stripe.com → Sign up with advigrow@gmail.com → Activate business (EIN or SSN, bank, ID, business address, support email orders@advigrow.online).
...
- Fastest (free): formspree.io → New Form → destination orders@advigrow.online → paste endpoint into `js/checkout.js` FORMSPREE_ENDPOINT and into contact form (same pattern).
- Alternative: Basin / Web3Forms, same one-line change.
- Until then: checkout falls back to `mailto:orders@advigrow.online` with prefilled JSON.
- Gmail filter: Settings → Filters → to:orders@advigrow.online → label Inquiry, forward to phone for urgent.
